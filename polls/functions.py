...
data=pd.read_csv(os.path.join(BASE_DIR ,"polls/static/data/food.csv"))
...
def Weight_Loss(age,weight,height):
    ...
    datafin=pd.read_csv(os.path.join(BASE_DIR ,"polls/static/data/nutrition_distriution.csv"))
    ...
    X_test=np.zeros((len(weightlosscat)*5,6),dtype=np.float32)
    ...
    X_tr, X_ts, y_tr, y_ts = train_test_split(X_train, y_train, test_size=0.4, shuffle=True)
...
def Healthy(age,weight,height):
    ...
    # print('healthycat',healthycat)
    # print('len_healthycat',len(healthycat))
    ...
    for zz in range(5):
        ...
        for jj in range(len(healthycat)):
            ...
            valloc.append(bmicls[zz])
            valloc.append(agecls[zz])
            # print('valloc'+'[' + str(jj) + ']',valloc)
            # print('len_valloc',len(valloc))
            healthycatfin[s]=np.array(valloc)
            ys.append(dnrlbl[jj])
            s+=1
    ...
    # print('healthycatfin',healthycatfin)
    # print('len_healthycatfin',len(healthycatfin))
    # print('ys',ys)
    # print('len_ys',len(ys))
    ...
    X_tr, X_ts, y_tr, y_ts = train_test_split(X_train, y_train, test_size=0.33, shuffle=True)
    ...
    print(y_pred)
    # large_diet = 25
    # if len(y_pred) >= large_diet:
    #     findData = random.sample(list(y_pred), large_diet)
    # else:
    #     findData = y_pred
